/*
 * Copyright (c) 2020-2025 Works Applications Co., Ltd.
 *
...
public class SudachiCompletionFilter(input: TokenStream, mode: Mode = DEFAULT_MODE) :
  ...
  private class CompletionTokenGenerator(mode: Mode) : Iterator<CompletionToken> {
    ...
    private fun Char.isAnyLowercaseAlphabet(): Boolean = this.isHalfWidthLowercase() || this.isFullWidthLowercase()
    ...
    public fun convertHiraganaToKatakana(input: String): String {
      return input.map { ch ->
        if (ch.isHiragana() && ch != '\u309f') { // 0x309f is Hiragana Digraph Yori, typically not mapped or mapped separately
          (ch.code + 0x60).toChar()
        } else {
          ch
        }
